...
public abstract class BaseApiController {
	...
	protected Claims getClaims() {
		...
		try {
			...
			if (expiration.before(new Date())) {
				...
				//缓存token是否已被处理
				String cacheToken = (String)redisUtil.get(String.format(CommonCacheConstant.API_USER_TOKEN_PREFIX, claims.getId()));
				if (cacheToken == null) {
					throw new BonelfException(CommonBizExceptionEnum.NOT_LOGIN);
				}
	...
	private void putNewToken2Resp(HttpServletResponse response, Claims cacheClaims) {
		if (cacheClaims.get(AuthConstant.REFRESH_CLAIM_FLAG) != null) {
			...
			redisUtil.set(String.format(CommonCacheConstant.API_USER_TOKEN_PREFIX, cacheClaims.getId()), newAuthorization, AuthConstant.REFRESH_SECOND);
